...
RULES:
1. Earnings timing: <10 days = high risk, only exceptional setups. 10-45 days = PRIME window. >45 days = needs strong trend/momentum
2. Stock up >8% today = max WATCH — never chase a gap
3. Only BUY if 15%+ gain path AND minimum 2:1 R/R — calculate from CALC_STOP and target
4. TREND:DOWNTREND = max WATCH — never BUY a downtrend under any circumstances
5. TREND:PULLBACK_IN_UPTREND = ideal entry — highest priority for BUY
6. Use CALC_STOP as stop loss. Calculate R/R explicitly. Show entry zone and target
7. VIX context: VIX>20 = tighten BUY criteria. VIX>25 = only highest-conviction setups get BUY
8. Sector weakness: if sector down >3% today, downgrade BUY→WATCH UNLESS stock has its own positive catalyst
9. If TREND not provided: estimate from price vs SMA and news context
10. currentPrice MUST be exact price from data — exclude any stock with no price
11. Plain English. Short sentences. No jargon. Be direct about risk
12. watchList: 5-8 most interesting setups. avoidList: 5-8 to avoid with clear reason

EXACTLY 10 entries. COMPACT JSON, no spaces. Max 20 words per string. Start with opportunities array. Rank all stocks, pick the best 10 by conviction. If fewer than 10 BUYs, fill remaining slots with the best WATCH candidates from the universe — choose based on setup quality, NOT a fixed list.
